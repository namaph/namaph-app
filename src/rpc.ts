...
export const execute = async (
	...
	console.log('multisigProgram', multisigProgram.programId.toBase58());
	...
	const remainingAccounts = accounts
		.map(a => {
			...
			if(a.isSigner){
				console.log('signer:', a.pubkey.toBase58());
			}

			return a.pubkey.equals(signer) ? { ...a, isSigner: false } : a
		})
		.concat({
			pubkey: pid,
			isSigner: false,
...
export const updateTopology = async (
	...
	console.log('update topology - signer:', signer.toBase58());
...
export const addMember = async (
	...
	let [newMembership] = await PublicKey.findProgramAddress([Buffer.from('membership'), multisig.toBytes(), newUser.toBytes()], programs.namaph.programId);
...
export const removeMember = async (
	...
	const { proposer, multisig, programs } = mTx;
	const multisigData = await programs.multisig.account.multisig.fetch(multisig);
	const owners = multisigData.owners as PublicKey[];
